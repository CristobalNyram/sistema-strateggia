...
$select_mes = mysqli_query($link, "SELECT DATE_FORMAT(fecha, '%m-%Y') AS mes FROM test_general where fecha between '$fecha_ini' and '$fecha_final' GROUP BY mes");
...
if($row > 0)
{
    while($arr = mysqli_fetch_array($select_mes))
    {
        ...
        $select2 = mysqli_query($link, "select count(id) as id from test_general where MONTH(fecha) = '$mes' and YEAR(fecha) = '$anio'");
