...
$select_mes = mysqli_query($link, "SELECT distinct(user) as user FROM test_general where fecha between '$fecha_ini' and '$fecha_final'");
...
if($row > 0)
{
    while($arr = mysqli_fetch_array($select_mes))
    {
        ...
        $select2 = mysqli_query($link, "select count(id) as id from test_general where user = '$user'");
...
<script>
$(function () {
        $('#grafica_usuario').highcharts({
            chart: {
                type: 'column'
            },
            title: {
                text: 'Gráfica de Cantidad de Estudios por Agente'
            },
            subtitle: {
                text: ''
            },
